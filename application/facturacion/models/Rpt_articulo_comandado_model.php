...
class Rpt_articulo_comandado_model extends General_model
{
    ...
    {
        $this->db->select('GROUP_CONCAT(DISTINCT b.comanda SEPARATOR ", ") AS comanda, TRIM(CONCAT(TRIM(s.nombre), IFNULL(CONCAT(" (", s.alias, ")"), ""))) AS sede, d.descripcion AS articulo, SUM(a.cantidad) AS cantidad, SUM(a.total) AS total, f.descripcion AS turno_tipo');
